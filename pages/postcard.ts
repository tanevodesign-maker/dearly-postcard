...
type FrameColor = [number, number, number]
...
function createFrameEl(): HTMLDivElement {
  ...
  // Block pointer events from reaching the drag logic
  uploadButton.addEventListener('pointerdown', (e) => e.stopPropagation())
  uploadButton.addEventListener('pointerup', (e) => e.stopPropagation())
...
const W0 = stage.clientWidth || 800
const H0 = stage.clientHeight || 500
...
const domCache = {
  ...
  frames: frameDefs.map(() => createFrameEl()), // cache lifetime: same as orb defs
}

const st: AppState = {
  frames: frameDefs.map(definition => ({
    x: definition.fx * W0,
    y: definition.fy * H0,
    w: definition.w,
    h: definition.h,
...
function hitTestFrames(frames: PhotoFrame[], px: number, py: number, activeCount: number): number {
  for (let index = activeCount - 1; index >= 0; index--) {
    const frame = frames[index]!
    const hw = frame.w * 0.5
    const hh = frame.h * 0.5
    if (px >= frame.x - hw && px <= frame.x + hw && py >= frame.y - hh && py <= frame.y + hh) {
      return index
    }
  }
  return -1
...
stage.addEventListener('pointerdown', event => {
  ...
  const activeFrameCount = st.frames.length
  const hitFrameIndex = hitTestFrames(st.frames, event.clientX, event.clientY, activeFrameCount)
  if (hitFrameIndex !== -1) {
...
function render(now: number): boolean {
  ...
  if (st.events.pointerDown !== null) {
    ...
    pointer = down
    if (drag === null) {
      const frameIndex = hitTestFrames(frames, down.x, down.y, activeFrameCount)
      if (frameIndex !== -1) {
  ...
  if (st.events.pointerMove !== null) {
    ...
    pointer = move
    if (drag !== null) {
      const frame = frames[drag.frameIndex]!
      frame.x = drag.startFrameX + (move.x - drag.startPointerX)
      frame.y = drag.startFrameY + (move.y - drag.startPointerY)
    }
  }
  ...
  if (st.events.pointerUp !== null) {
    ...
    if (drag !== null) {
      const frame = frames[drag.frameIndex]!
      frame.x = drag.startFrameX + (up.x - drag.startPointerX)
      frame.y = drag.startFrameY + (up.y - drag.startPointerY)
      drag = null
    }
  }
  ...
  for (let index = 0; index < activeFrameCount; index++) {
    const frame = frames[index]!
    const hw = frame.w * 0.5
    const hh = frame.h * 0.5
    
    // Boundary clamping
    if (frame.x - hw < 0) frame.x = hw
    if (frame.x + hw > pageWidth) frame.x = pageWidth - hw
    if (frame.y - hh < 0) frame.y = hh
    if (frame.y + hh > pageHeight) frame.y = pageHeight - hh
  ...
  const bodyTop = gutter + (isNarrow ? 14 : 20)
  ...
  for (let columnIndex = 0; columnIndex < columnCount; columnIndex++) {
    ...
    for (let index = 0; index < activeFrameCount; index++) {
      const frame = frames[index]!
      rects.push({
        x: frame.x - frame.w * 0.5 - (isNarrow ? 4 : 8),
        y: frame.y - frame.h * 0.5 - (isNarrow ? 2 : 4),
        w: frame.w + (isNarrow ? 8 : 16),
        h: frame.h + (isNarrow ? 4 : 8),
      })
    }
  ...
  const hoveredFrameIndex = hitTestFrames(frames, pointer.x, pointer.y, activeFrameCount)
  const cursorStyle = drag !== null ? 'grabbing' : hoveredFrameIndex !== -1 ? 'grab' : ''
  ...
  for (let index = 0; index < domCache.frames.length; index++) {
    const frame = frames[index]!
    const element = domCache.frames[index]!
    if (index >= activeFrameCount) {
      element.style.display = 'none'
      continue
    }
    element.style.display = ''
    element.style.left = `${frame.x - frame.w * 0.5}px`
    element.style.top = `${frame.y - frame.h * 0.5}px`
